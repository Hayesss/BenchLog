...
- 后端（D2）：`sample.batchCreate`——行扫描自动排孔（A1→A2→…，与 Benchling 默认一致），空位不足整体拒绝不部分入库；recordId 关联来源记录（归属校验排除软删）；单批上限 96；返回盒名+孔位坐标清单
- 前端（D2 表格转样本）：光标在表格内时 inTable 工具组出现「入库」→ 解析当前 TipTap 表格（含表头行）→ `TableImportDialog` 列映射（名称/类型/浓度/体积/日期/忽略，按表头自动猜，单值角色互斥）+「首行是表头」开关 + 数据预览（名称为空行置灰将跳过）+ 目标盒选择（显示空位/容量）→ ensureRecordId 先落库 → 批量创建；类型值模糊归一 9 种 enum，日期支持 `YYYY/M/D`、`YYYY.M.D` 归一到 ISO，空日期列回落记录日期；成功态展示孔位 chips
- 前端（D1 图片标注）：选中正文图片（NodeSelection）时工具栏出现「标注」→ `ImageAnnotateDialog` 纯 canvas 自绘零依赖——画笔/箭头/矩形/文字（白描边保可读）+ 6 色低饱和色板 + 撤销（30 步 ImageData 快照栈）/清空 + 线宽字号随原图自适应 + CSS 缩放坐标换算；保存 PNG dataURL 经 `updateAttributes('image')` 回写节点，自动保存入库闭环
- 验证：`npx tsc -b` 通过；冒烟 7 断言真实库全 PASS（A1-A3 顺序/recordId 关联+recordTitle 回联/接续 B1,B2/空位不足整体拒绝/归属校验/末孔补满，脚本跑完即删）；**playwright 生产实测 21/21**（建盒→插入 3×3 表格 Tab 填数→入库 dialog 列映射→成功态孔位 chips→getBox 验证关联+类型归一+孔位→上传图片→选中出标注按钮→canvas 画笔+箭头+撤销→保存回写 PNG dataURL→自动保存无报错）；33 资产 gzip+brotli 预压缩；入包验证 batchCreate/空位不足/入库/标注齐在；测试数据已清理
- 范围：合并单元格按 textContent 简化取值；文字标注用 prompt 输入（v1）；标注不含拖拽移动已有图形
